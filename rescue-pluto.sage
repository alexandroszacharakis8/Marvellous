...
def permutation_round_keys():
    ...
    key_state = matrix(rescue_pluto.F, [[rescue_pluto.F.zero()]] * rescue_pluto.m)

    key_injection = rescue_pluto.initial_constant
    ...
    for r in range(0, 2 * rescue_pluto.Nb):

        # constants for inverse SBOX
        if r % 2 == 0:
            for i in range(0,rescue_pluto.m):
                key_state[i,0] = key_state[i,0]^rescue_pluto.invalpha

        # constants for pow5 SBOX
        else:
            for i in range(0,rescue_pluto.m):
                key_state[i,0] = key_state[i,0]^rescue_pluto.alpha
        key_injection = rescue_pluto.constants_matrix * key_injection + rescue_pluto.constants_constant

        key_state = rescue_pluto.MDS * key_state + key_injection
...
def random_input():
    r = []
    for i in range(4):
      r.append([rescue_pluto.F.random_element()])
    return matrix(rescue_pluto.F, r)
...
# Set a fixed 0 key
fixed_key = matrix(rescue_pluto.F, [[rescue_pluto.F.zero()]] * rescue_pluto.m)
zero_state = matrix(rescue_pluto.F, [[rescue_pluto.F.zero()]] * rescue_pluto.m)

pairs_raw = []

# compute output of [0,0,0,0]
pairs_raw.append((zero_state, rescue_pluto.BlockCipher(fixed_key, zero_state)))

# compute the output for the random pairs
pairs_raw.extend(list(map(lambda x: (x, rescue_pluto.BlockCipher(fixed_key, x)), inputs)))
...
# compute the output for the random pairs
keyed_pairs_raw.extend(list(map(lambda x: (x[0], x[1], rescue_pluto.BlockCipher(x[0], x[1])), inputs)))

# Sponge Test Vectors. We sample 4 vectors of size 12. We use rate 3.
sponge_inputs_raw = [[rescue_pluto.F.random_element() for _ in range(12)] for _ in range(4)]
sponge_inputs = [list(map(convert_limbs, x)) for x in sponge_inputs_raw]
sponge_outputs_raw = [rescue_pluto.Sponge(input, 3) for input in sponge_inputs_raw]
...
for i in range(32):
    chain_state = rescue_pluto.BlockCipher(fixed_key, chain_state)
...
input = matrix(rescue_pluto.F, [[rescue_pluto.F.zero()]] * rescue_pluto.m)

outputs = copy(messages)

# Encrypt
for msg in outputs:
    key_stream = rescue_pluto.BlockCipher(key, input)
    for i in range(4):
        msg[i] += key_stream[i]

    input[0,0] += rescue_pluto.F(1)
...
print("const MDS: RescueMatrix<Fq> = [")
for out in mds:
    print("[")
    for ss in out:
...
print("const RC_VECTOR: StateVectorsMatrix<Fq> = [")
for out in rc:
    print("[")
    for ss in out:
        print_scalar_slice(ss)
...
print("const KI_VECTOR: StateVectorsMatrix<Fq> = [")
for out in ki:
    print("[")
    for ss in out:
        print_scalar_slice(ss)
...
print("pub(crate) const PLUTO_TEST_VECTORS: [InputOutputPair<Fq>; 5] = [")
for pair in pairs:
    print("([")
    for val in pair[0]:
...
print("pub(crate) const PLUTO_TEST_VECTORS_KEYED: [KeyedInputOutputPair<Fq>; 4] = [")
...
print("pub(crate) const PLUTO_SPONGE_TEST_VECTORS: [([Fq; 12], [Fq; 3]); 4] = [")
